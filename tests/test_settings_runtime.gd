...
func _test_audio_bus_change() -> void:
	...
	if music_slider == null:
		_settings_menu.queue_free()
		_tf.run_test("AudioServer bus 音量失败", func() -> Dictionary:
			return {"pass": false, "message": "MusicSlider 失败"})
		return
	...
	if music_idx < 0:
		_settings_menu.queue_free()
		_tf.run_test("AudioServer bus 音量失败", func() -> Dictionary:
			return {"pass": false, "message": "Music bus 失败"})
		return
...
func _test_persistence_across_instances() -> void:
	...
	if sfx_slider == null:
		menu1.queue_free()
		_tf.run_test("Settings 失败", func() -> Dictionary:
			return {"pass": false, "message": "SFXSlider 失败"})
		return
	...
	if sfx_slider2 == null:
		menu2.queue_free()
		_tf.run_test("Settings 失败", func() -> Dictionary:
			return {"pass": false, "message": "SFXSlider 2 失败"})
		return
	...
	_tf.run_test("Settings 失败", func() -> Dictionary:
		return {"pass": passed, "message": "新实例失败 = %.2f (期望 0.65)" % restored_value})
...
func _test_config_file_format() -> void:
	...
	_tf.run_test("settings.cfg 失败", func() -> Dictionary:
		return {"pass": passed, "message": msg})
